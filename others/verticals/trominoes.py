"""
Visual proof of Tiling with trominoes.
Proofs without Words II. Roger B. Nelsen. p. 123.
"""
import numpy as np
...
class Trio(MovingCameraScene):
    def construct(self):
        ...
        # Introduction text
        txt_title = [
            Tex(r"Pavage avec ", font_size=48, color=BLACK),
            Tex(r"des triominos", font_size=48, color=BLACK),
        ]
        txt_title = VGroup(*txt_title).arrange(DOWN).move_to([0, 2, 0])

        txt = [
            Tex(r"Démonstration", font_size=36, color=BLACK),
            Tex(r"Solomon W. Golomb", font_size=28, color=BLACK)
        ]
        ...
        ref = [
            ...
            Tex(r"Monthly, vol. 61, no. 10,", font_size=30, color=BLACK),
            Tex(r"(Dec. 1954), pp. 675-682", font_size=30, color=BLACK),
        ]
        ref = VGroup(*ref)\
