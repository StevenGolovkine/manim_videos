...
class Pythagorean(MovingCameraScene):
    def construct(self):
        ...
        txt = [
            Tex(r"Démonstration II", font_size=36, color=BLACK),
        ...
        txt_angle = Tex(r"$2\pi/3$", font_size=18, color=BLACK).\
            next_to(angle, UP, buff=0.05)
        self.play(
            Create(triangle),
            Write(txt_triangle),
            Create(angle),
        ...
        # Write formula
        formula = Tex(
            r"$T_\beta = T + T_\alpha + T_\gamma$", font_size=30, color=BLACK
        ).move_to([0, 3, 0])
        self.play(Write(formula))

        # Move everything to the top
        self.play(
            all_object.animate.move_to([0, 3, 0]),
        ...
        self.play(
            triangle_BCE_copy.animate.move_to([0, -1.5, 0]),
            run_time=0.5
        )
        ...
        self.play(
            triangle_copy_1.animate.move_to(triangle_BCE_copy.get_center_of_mass() + [-0.27, -0.72, 0]),
            run_time=0.5
        )

        self.play(
            triangle_copy_2.animate.rotate(-120 * DEGREES),
            run_time=0.5
        )
        self.play(
            triangle_copy_2.animate.move_to(triangle_BCE_copy.get_center_of_mass() + [-0.51, 0.55, 0]),
            run_time=0.5
        )

        self.play(
            triangle_copy_3.animate.rotate(120 * DEGREES),
            run_time=0.5
        )
        self.play(
            triangle_copy_3.animate.move_to(triangle_BCE_copy.get_center_of_mass() + [0.96, 0.15, 0]),
            run_time=0.5
        )
        ...
        triangle_copy_4 = triangle_copy_3.copy().set_opacity(0.5)

        self.play(
            triangle_copy_4.animate.rotate(-180 * DEGREES).move_to(triangle_copy_3.get_center_of_mass() + [-0.47, 0.18, 0]),
            run_time=1
        )
        txt_triangle_copy_4 = txt_triangle_copy_3.copy().move_to(triangle_copy_4.get_center_of_mass())
        self.play(
            Write(txt_triangle_copy_4),
            run_time=0.5
        )

        triangle_copy_5 = triangle_copy_4.copy().set_opacity(0.5)
        
        self.play(
            triangle_copy_5.animate.flip(LEFT).move_to(triangle_copy_3.get_center_of_mass() + [-1.95, 0.16, 0]),
            run_time=1
        )

        txt_triangle_copy_5 = txt_triangle_copy_3.copy().move_to(triangle_copy_5.get_center_of_mass())
        self.play(
            Write(txt_triangle_copy_5),
            run_time=0.5
        )

        triangle_copy_6 = triangle_copy_5.copy().set_opacity(0.5)
        self.play(
            triangle_copy_6.animate.rotate(180 * DEGREES).move_to(triangle_copy_5.get_center_of_mass() + [0.47, 0.18, 0]),
            run_time=1
        )
        txt_triangle_copy_6 = txt_triangle_copy_5.copy().move_to(triangle_copy_6.get_center_of_mass())
        self.play(
            Write(txt_triangle_copy_6),
            run_time=0.5
        )
        
        self.play(
            triangle_ABF_copy.animate.rotate(-60 * DEGREES).move_to(triangle_BCE_copy.get_center_of_mass() + [-0.01, -0.25, 0]),
            run_time=1
        )
        ...
        ref = [
            ...
            Tex(r"vol. 35, no. 3,", font_size=30, color=BLACK),
            Tex(r"(March 2004), p. 215.", font_size=30, color=BLACK)
        ]
        ref = VGroup(*ref)\
