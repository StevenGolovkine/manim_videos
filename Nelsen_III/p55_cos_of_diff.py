"""
Visual proof of the cosine of the difference formula
...
class Sum(MovingCameraScene):
    def construct(self):
        ...
        # Introduction text
        txt_title = [
            Tex(r"Le cosinus de", font_size=48, color=BLACK),
            Tex(r"la différence de", font_size=48, color=BLACK),
        ...
        txt = [
            ...
            Tex(r"David Richeson", font_size=28, color=BLACK)
        ]
        ...
        # Figure
        formula = Tex(
            r"$\cos(u-v)=\cos u\cos v+\sin u\sin v$",
        ...
        angle_u = Angle(
            altitude, right_hypotenuse,
            radius=0.32, color=BLACK, stroke_width=2
        )

        labels = VGroup(
            Tex(r"$v$", font_size=25, color=BLACK).next_to(angle_v, RIGHT, buff=0.08),
            Tex(r"$u$", font_size=25, color=BLACK).next_to(angle_u, RIGHT + DOWN, buff=0.08),
        )
        ...
        if theta_PA < theta_PC:
            theta_PA += 2 * PI
        angle_sum = Angle(
            left_hypotenuse, left_outer,
            radius=0.36, color=BLACK, stroke_width=2
        )

        angle_sum_label = Tex(r"$u-v$", font_size=18, color=BLACK).\
            next_to(angle_sum, UP + RIGHT, buff=0.12)
        ...
        self.play(
            Write(
                side_label(
                    r"$\cos u$", A, P, offset=-0.1, side=1,
                    font_size=18, t=0.6
                )
            ),
            Write(
                side_label(
                    r"$\cos(u-v)\cdot\cos u$",
        ...
        self.play(
            Write(
                Tex(r"$\cos u\cdot\sin v$", font_size=18, color=BLACK)
                    .rotate(PI / 2)
                    .move_to(altitude.get_center() + 0.2 * LEFT + 0.03 * DOWN)
            ),
            Write(
                Tex(r"$\cos u\cdot\cos v$", font_size=18, color=BLACK)
        ...
        arrow_label = Tex(
            r"$\cos(u-v)$", font_size=27, color=BLACK
        ...
        ref = [
            ...
            Tex(r"vol. 45, no. 5 (Nov. 2014),", font_size=30, color=BLACK),
